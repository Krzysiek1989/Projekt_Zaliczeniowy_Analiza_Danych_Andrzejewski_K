...
def generate_active_shop_graph(df: pd.DataFrame) -> None:
    ...
    wrkbook = xlsxwriter.Workbook('.\\output\\LH'
    ...
    lista_sr = list_of_shops_df['Nazwa_Spolki'].unique()
    for sr in lista_sr:
...
def generate_history_graph(sr: str, df: pd.DataFrame, wrkbook: xlsxwriter.Workbook):
    ...
    fig.savefig(imgdata, format='png')
    worksheet.insert_image(0, 0, '', {'image_data': imgdata})
    save_copy_for_sr(spolka, imgdata, f'Historia sklepow sieci Lewiatan {spolka}')
...
if __name__ == "__main__":
    list_of_shop_file = (".\\source\\shop_list"
                         "\\list_of_shops.csv")
    list_of_shops_df = load_df_from_csv(list_of_shop_file)
    list_of_shops_df = clean_shop_df(list_of_shops_df)
    list_of_shops_df = format_and_fill_shop_list_df(list_of_shops_df)
    lista_spolek = list_of_shops_df['Nazwa_Spolki'].unique()
    workbook = xlsxwriter.Workbook('.\\output\\LH'
    ...
    for spolka in lista_spolek:
        ...
        generate_history_graph(spolka, list_of_shops_df, workbook)
        save_shop_list_for_SR(spolka, list_of_shops_df[list_of_shops_df['Nazwa_Spolki'] == spolka].drop(columns=['ws_year', 'ws_month', 'wy_year', 'wy_month']))
    workbook.close()
    generate_active_shop_graph(list_of_shops_df)
